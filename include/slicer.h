...
struct Triangle
{
	...
	//         -2 = error in function
	int
	intersectPlane(const Plane& plane, LineSegment& ls) const
	{
...
public:
	...
	{
		return v3(upperRightVertex.x - bottomLeftVertex.x, upperRightVertex.y -
		bottomLeftVertex.y, upperRightVertex.z - bottomLeftVertex.z);
	}
...
int
stlToMeshInMemory(const char* stlFile,
	TriangleMesh* mesh, bool isBinaryFormat)
{
	if (!isBinaryFormat) {
		ifstream in(stlFile);
		if (!in.good()) return 1;
	...
	const v3& aabbSize)
{
	...
	glm::tquat<float> quaternion = glm::tquat<float>(DEG_TO_RAD(eulerAngles)); 
...
slice(string& modelFileName) 
{
	...
	TriangleMesh mesh;
	if (stlToMeshInMemory(modelFileName.c_str(), &mesh, isBinaryFormat) != 0)
		return false;

	cerr << "Mesh has " << mesh.size() << " triangles" << endl;
