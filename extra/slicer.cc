...
struct v3
{
	...
	void transform(const glm::mat4 &mat) { glm::vec4 v = glm::vec4(x, y, z, 1.0f), vt;
		vt = mat * v; x = vt.x; y = vt.y; z = vt.z; }
...
int
stlToMeshInMemory(const char* stlFile, TriangleMesh* mesh, bool isBinaryFormat)
{
	if (!isBinaryFormat) {
		ifstream in(stlFile);
...
// each slice
int
triMeshSlicer(const TriangleMesh* mesh,std::vector<std::vector<LineSegment>>& slicesWithLineSegments,
...
int
exportSingleGIVFormat(const std::vector<std::vector<LineSegment>>& slicesWithLineSegments, const v3& aabbSize)
{
	//char filename[256];
	FILE* f = NULL;
	float dx = 0, dy = 0;
...
int
exportSingleGIVFormat3D(const std::vector<std::vector<LineSegment>> &slicesWithLineSegments, const v3 &aabbSize)
{
...
main(int argc, char** argv) 
{
	...
	fprintf(stderr, "Mesh has %lu triangles\n", mesh.size());
