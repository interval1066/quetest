...
// each slice
int triMeshSlicer(const TriangleMesh* mesh,std::vector<std::vector<LineSegment>>& slicesWithLineSegments,
	...
	const v3                                    &aabbSize) {
		...
		for (size_t j=0; j<lss.size(); ++j) {
			...
			printf("\n%f %f %f", dx+lss[j].v[0].x, dy+lss[j].v[0].y, dy+lss[j].v[0].z);
			printf("\n%f %f", dx+lss[j].v[1].x, dy+lss[j].v[1].y);
		}
	}
	fclose(f);
...
// Generate a 3D View using OpenGL Math Library
	...
	for (size_t i=0; i<nSlices; ++i) {
		...
		for (size_t j=0; j<lss.size(); ++j) {
			...
			//fprintf(f, "\n%f %f", p0.x, p0.y);
			//fprintf(f, "\n%f %f", p1.x, p1.y);
			printf("\n%f %f %f", p0.x, p0.y, p0.z);
			printf("\n%f %f %f ", p1.x, p1.y, p1.z);
		}
		std::cout << std::endl;
	}
	fclose(f);
